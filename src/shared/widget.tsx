...
export function createTsushinWidget(config: ViewConfig) {
  return {
...
function buildClassName(config: ViewConfig): string {
  return `
    top: ${config.top};
    left: ${config.left};
    color: ${DOWN_COLOR};
...
function buildCommand(widgetDir: string): string {
  return `
...
function parseSample(output: string): Sample {
  const parsed = JSON.parse(output.trim()) as Partial<Sample>;

  if (
    typeof parsed.interfaceName !== "string" ||
...
function renderWidget(state: WidgetState, config: ViewConfig) {
  ...
  return (
    ...
    >
      ...
      <svg
        height={chartTop + chartHeight + chartBottom}
        style={{ left: 0, position: "absolute", top: 0 }}
        width={config.width}
      >
        {gridValues.map((gridValue) => (
          <line
            key={`grid-${gridValue.y}`}
            stroke={GRID_COLOR}
            strokeWidth={1}
            x1={chartLeft}
        ...
        {config.showTimeLabels && firstSample && lastSample ? (
          <text fill={MUTED_COLOR} fontSize={10} textAnchor="start" x={chartLeft} y={config.height - 8}>
            {timeFormatter.format(firstSample.timestamp)}
          </text>
        ) : null}
        {config.showTimeLabels && firstSample && lastSample ? (
          <text fill={MUTED_COLOR} fontSize={10} textAnchor="end" x={chartLeft + chartWidth} y={config.height - 8}>
...
function renderMetric(label: string, value: number, color: string) {
  return (
...
function getMaxRate(samples: Sample[]): number {
...
function roundUpForScale(value: number): number {
  if (value <= 0) {
    return MIN_SCALE_KB;
  }
